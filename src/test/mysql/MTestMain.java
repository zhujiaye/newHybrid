...
public class MTestMain {
	...
	public static void main(String[] args) {
		...
		if (args.length > 3) {
			...
			if (tmp == 0)
				USE_VIRTUAL_TENANT = false;
			else
				USE_VIRTUAL_TENANT = true;
		}

		if (USE_VIRTUAL_TENANT)
			DoTest_V();
		else
			DoTest();
	}
	...
	public static void DoTest() {
		try {
			...
			BufferedWriter out = new BufferedWriter(new FileWriter("MTest.txt",
					true));
			...
			System.out.println("\tTime per interval: " + TIME_PER_INTERVAL
					/ 60000 + " minutes.");
			// run test
			...
			for (int interval = 0; interval < TOTAL_INTERVAL; interval++) {
				...
				System.out.println("Interval " + (interval + 1)
						+ " finished! Throughput: " + query + ".(Total: "
						+ TOTAL_INTERVAL + " intervals...)");
				System.out.println("Read: " + read + ". Write: " + write
						+ ". Write percent: "
						+ df.format(write * 1.0 / (read + write)));
			}
